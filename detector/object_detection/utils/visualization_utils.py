...
def get_box_dimensions_and_text(
    image,
    boxes,
    classes,
    scores,
  ...
  for i in range(min(max_boxes_to_draw, boxes.shape[0])):
    if scores is None or scores[i] > min_score_thresh:
      box = tuple(boxes[i].tolist()) # creating box for found detection
  ...
  objects_dimensions_and_info = []
  # Get dimensions for every item's box.
